...
namespace UdonSharp.Video
{
    ...
    {
        #region Central Media Library
        public CentralMediaLibrary mediaLibrary;
        public TextMeshProUGUI[] mediaLibraryLabels;
        [UdonSynced] int _libraryCategory, _libraryIndex, _librarySeason, _libraryChapter;

        public void SetLibraryIndex(int category, int index, int season = -1, int chapter = -1)
        {
            _libraryCategory = category;
            _libraryIndex = index;
            _librarySeason = season;
            _libraryChapter = chapter;
        }
        public void UpdateLibraryLabel()
        {
            if (mediaLibrary == null || mediaLibraryLabels == null || mediaLibraryLabels.Length == 0) return;

            string label = string.Empty;

            if (_libraryCategory > -1 && _libraryIndex > -1)
            {
                label = mediaLibrary.Categories[_libraryCategory].TitleMatrix[_libraryIndex];
                if (_librarySeason > -1)
                    label = label + $" • Temp {_librarySeason + 1}";
                if (_libraryChapter > -1)
                    label = label + $" • Cap {_libraryChapter + 1}";
            }

            foreach (TextMeshProUGUI mediaLabel in mediaLibraryLabels)
                if (Utilities.IsValid(mediaLabel)) mediaLabel.text = label;
        }
        void PlayNextVideoFromLibrary()
        {
            if (!Networking.IsOwner(gameObject)) return;

            if (_libraryIndex > -1 && _librarySeason < 0)
            {
                _libraryCategory = -1;
                _libraryIndex = -1;
                _librarySeason = -1;
            ...
            if (_libraryCategory < 0 || _libraryIndex < 0 || _librarySeason < 0 || _libraryChapter < 0)
                return;

            _libraryChapter++;
            VRCUrl url = mediaLibrary.Categories[_libraryCategory]
                .GetChapterAt(_libraryIndex, _librarySeason, _libraryChapter);

            if (url == null)
            {
                _librarySeason++; // Try and change seasons
                _libraryChapter = 0;

                url = mediaLibrary.Categories[_libraryCategory]
                    .GetChapterAt(_libraryIndex, _librarySeason, _libraryChapter);

                if (url == null)
                {
                    // Media playing has finally ended
                    _libraryCategory = -1;
                    _libraryIndex = -1;
                    _librarySeason = -1;
                    _libraryChapter = -1;
...
#pragma warning restore CS0414
            ...
            if (_videoSync)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            _waitForSync = true;
                            SetStatusText("Esperando owner sync...");
                        }
                        else
                        {
                            _waitForSync = false;
        ...
        public override void OnVideoError(VideoError videoError)
        {
            ...
            {
                SetStatusText("Limitado, reintentando...");
            ...
            {
                SetStatusText("Error de video, reintentando...");
                LogError("Video player error when trying to load url.");
            ...
            LogError($"Video failed to play with error {videoError}");

            switch (videoError)
            {
                case VideoError.InvalidURL:
                    SetStatusText("URL Inválida");
                    break;
                case VideoError.AccessDenied:
                    SetStatusText("Video bloqueado, activar Untrusted URLs");
                    break;
                default:
                    SetStatusText("No se pudo cargar el video");
                    break;
            }
