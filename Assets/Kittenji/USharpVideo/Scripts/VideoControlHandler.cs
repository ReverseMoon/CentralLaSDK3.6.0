...
#pragma warning restore CS0649
        ...
        [Header("Kittenji")]
        [SerializeField] bool manualRegister;

        private void Start()
        {
            Debug.Log("[USharpVideo] Starting...");
        }
        ...
        private void OnDisable()
        {
            ...
            if (manualRegister) targetVideoPlayer = null;
        }
        ...
        public void SetLocked(bool locked)
        {
            if (locked)
            {
                ...
                if (Networking.IsOwner(targetVideoPlayer.gameObject) || targetVideoPlayer.CanControlVideoPlayer())
                {
                    ...
                    if (urlFieldPlaceholderText) urlFieldPlaceholderText.text = "Insertar URL...";
                }
                else
                {
                    if (lockGraphic) lockGraphic.color = redGraphicColor;
                    if (urlFieldPlaceholderText) urlFieldPlaceholderText.text = $"Solo el master {Networking.GetOwner(targetVideoPlayer.gameObject).displayName} puede agregar URLs";
                }
            }
            else
            {
        ...
        void RunUIUpdate()
        {
            if (Utilities.IsValid(targetVideoPlayer) && targetVideoPlayer.IsInVideoMode())
            {
